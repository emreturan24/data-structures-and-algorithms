...
public class SceneManager {
    ...
    private record CachedEntry(Parent root, Object controller) {}
    private static final Map<String, CachedEntry> cache = new HashMap<>();
    ...
    public static Parent load(String fxmlFile) throws IOException {
        CachedEntry entry = cache.get(fxmlFile);
        if (entry != null) {
            return entry.root;
        }

        FXMLLoader loader = new FXMLLoader(SceneManager.class.getResource(BASE + fxmlFile));
        Parent root = loader.load();
        Object controller = loader.getController();
        cache.put(fxmlFile, new CachedEntry(root, controller));
        return root;
    }
